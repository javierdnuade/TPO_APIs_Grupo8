...
public class CobranzaServiceImpl implements CobranzaService {
    ...
    public CobranzaResponse registrar(CobranzaRequest request) {
        ...
        if (request.getImporte().compareTo(cuota.getCredito().getImporteCuota()) != 0 ) {
            throw new BusinessException
            ("El importe ingresado debe ser exactamente igual a la cuota.");
        }

        Cobranza cobranza = new Cobranza(null, cuota, request.getImporte(), LocalDate.now());
