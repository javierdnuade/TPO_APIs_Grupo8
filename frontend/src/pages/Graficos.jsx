...
import { getCobranzasMensuales } from '../api/dashboard';
...
export default function Graficos() {
  ...
  const [cobranzasMensuales, setCobranzasMensuales] = useState([]);
  const [cobranzasMensualesLoading, setCobranzasMensualesLoading] = useState(false);
  const [cobranzasMensualesError, setCobranzasMensualesError] = useState(null);
  ...
  const cargarCobranzasMensuales = async () => {

    setCobranzasMensualesError(null);
    setCobranzasMensualesLoading(true);

    try {
      const data = await getCobranzasMensuales();
      setCobranzasMensuales(data);
    } catch (error) {
      setCobranzasMensualesError(error.message);
    } finally {
      setCobranzasMensualesLoading(false);
    }
  };
  ...
  const cobranzasMensualesChart = useMemo(() => {
    return cobranzasMensuales.map(item => ({
      label: `${item.mes}/${item.anio}`,
      value: item.totalCobrado
  ...
  <h3 style={styles.h3}>
    Evolución de cobranzas por mes
  </h3>

  <p style={styles.subtitleSmall}>
    Visualiza cómo evolucionan las cobranzas mes a mes.
  </p>

  {cobranzasMensualesError && (
    <div style={styles.error}>
      {cobranzasMensualesError}
    </div>
  )}
  ...
  <div style={styles.row}>
        <button
          className="ui-btn"
          style={styles.btn}
          onClick={cargarCobranzasMensuales}
          disabled={cobranzasMensualesLoading}
        >

          {cobranzasMensualesLoading
            ? 'Cargando...'
            : 'Cargar evolución'}
      ...
      {cobranzasMensuales.length > 0 && (
        <div style={{ marginTop: 20 }}>

          <LineChart
            items={cobranzasMensualesChart}
          />
